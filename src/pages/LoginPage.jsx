  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignUp) {
        ...
        const signupResp = await fetch("http://localhost:9090/back2/users/insert", {
          method: "POST",
        ...
        const loginResp = await fetch("http://localhost:9090/back2/users/signin", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            email: formData.email,
        ...
        if (code === "200" && token) {
          ...
          try {
            const usernameResp = await fetch("http://localhost:9090/back2/users/getusername", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
