...
const LeaguesPage = () => {
    ...
    const handleDelete = async (id) => {
        await axios.delete(`http://localhost:9090/back2/api/leagues/${id}`);
        fetchLeagues();
    };

    const handleSave = async () => {
        if (currentLeague.id) {
            await axios.put(`http://localhost:9090/back2/api/leagues/${currentLeague.id}`, currentLeague);
        } else {
            await axios.post('http://localhost:9090/back2/api/leagues', currentLeague);
        }
        setOpen(false);
        fetchLeagues();
    };
