...
const LeaguesPage = () => {
    ...
    const fetchLeagues = async () => {
        const response = await axios.get('http://localhost:9090/api/leagues');
        setLeagues(response.data);
    };

    const handleDelete = async (id) => {
        await axios.delete(`http://localhost:9090/api/leagues/${id}`);
        fetchLeagues();
    };

    const handleSave = async () => {
        if (currentLeague.id) {
            await axios.put(`http://localhost:9090/api/leagues/${currentLeague.id}`, currentLeague);
        } else {
            await axios.post('http://localhost:9090/api/leagues', currentLeague);
        }
        setOpen(false);
        fetchLeagues();
    };
